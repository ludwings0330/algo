...
# Target 값보다 처음으로 크거나 같은 값이 나오는 위치를 찾는 알고리즘

# Target 값이 있을 때 -> 가장 왼쪽 값의 index 반환
# Target 값이 없을 때 -> target 보다 큰 가장 작은 정수의 index 반환
...
def lower_bound(arr, target):
    ...
    while left < right:
        ...
        # target 보다 작으면 arr[mid] 는 가능성이 없는 수 이므로 left = mid + 1 로 둔다
        if arr[mid] < target:
            left = mid + 1
        # 크거나 같으면 정답 가능성이 있기 떄문에 mid + 1 이 아니라, right = mid 로 둔다.
        # target이 존재하면 가장 왼쪽 값을 가져와야 하기 때문에 right = mid로 둔다
        elif arr[mid] >= target:
            right = mid
